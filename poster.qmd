...
* *ggplot2* 
  * Allow hover, drag, zoom and export
  * Ggplot2 allows an easier view for people with sight disabilities or difficulties, increasing size of town name text by zooming is great for increasing accessibility across all audience. 
* *shiny* 
  * Allow Sorting of graph to clearly differentiate categories. 
  * Dynamic input for example the distribution of only robbery crime though out the state.
* *plotly* 
  * Customized tool tip with ggplot2
  * Plotly can easily be used to expand long named town , show exact number while not crowding the poster. As the border of each town is separated by a thin gray line , darkening the borders while adding shadow will show lighted areas when hovered. 
