...
# Plot the top 5 crime codes in each area map with jitter points
top_crime_codes_by_area_map <- top_crime_codes_by_area %>%
  ...
  labs(title = "Number of Top 5 Crimes in Each Area", x = "Area", y = "Area Name", color = "Crime Type") +
  theme_minimal() +
  theme(
...
# Add jittered coordinates to top_crimes_la_count
set.seed(3007)  # For reproducibility
top_crimes_la_count <- top_crimes_la_count %>%
  mutate(
    jittered_long = long_center + rnorm(n(), mean = 0, sd = 3000),
    jittered_lat = lat_center + rnorm(n(), mean = 0, sd = 3000)
  )
...
top_crime_codes_by_area_map_sf <- ggplot() +
  ...
  geom_point(data = top_crimes_la_count, 
             aes(x = jittered_long, y = jittered_lat, size = Crime_Count, colour = factor(Crm.Cd.Desc)),
             alpha = 0.6) +
  ...
  scale_fill_viridis_c(name = "Total Crime Count",
                       ...
                       labels = c("30k", "40k", "50k", "60k"),
                       direction = -1) +
  scale_colour_viridis_d(name = "Crime Type",
                         option = "inferno",
                         begin = 0.2,
                         end = 0.8,
                         direction = -1) +
  ...
  guides(colour = guide_legend(override.aes = list(size = 6)))  # Adjust the size of the points in the legend
