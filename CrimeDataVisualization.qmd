...
top_crimes_la_count <- top_crimes_la_count %>%
  group_by(AREA) %>%
  mutate(
    rank = rank(Crm.Cd.Desc),
    jittered_long = long_center - 5000,
    jittered_lat = lat_center - 10000 + rank * 3750
  ) %>%
  ungroup()
```
...
top_crime_codes_by_area_map_sf <- ggplot() +
  ...
  geom_point(data = la_count, aes(x = long_center - 500, y = lat_center - 500), color = "black", size = 3) +
  geom_point(data = top_crimes_la_count, 
             aes(x = jittered_long , y = jittered_lat, shape = factor(Crm.Cd.Desc), size = 2)) +
