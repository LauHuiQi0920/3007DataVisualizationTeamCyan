...
cleaned_crime_data <- cleaned_crime_data %>%
  select(DATE.OCC, DATE.OCC.MONTH, DATE.OCC.YEAR, everything())
...
# Fix the AREA.NAME for areas with wrong names
...
```{r}
#| label: data-preparation - top 3 crimes by area
#| message: false

# Based of crime code, find top 3 crime codes and the corresponding crime description based on each area
...
###### <span style="color: red;">This visualisation below is for the team's reference only. Kindly ignore.</span>
```{r}
...
```{r}
#| label: data-visualisation - centroids
#| message: false
#| output: false

# Read the LA city boundary shapefile
la <- st_read("City_Boundary.geojson")

# Calculate the centroids of the LA city boundary
la_centroids <- la %>%
  st_centroid() %>%
...
```{r}
#| label: data-visualisation - jitter coords for top 3 crimes
#| message: false
...
```{r}
#| label: data-visualisation - jitter coords for top 3 crimes (alternative)
#| message: false

# Generate jittered coordinates for the top 3 crimes in each area
top_crimes_la_count <- top_crimes_la_count %>%
  group_by(AREA) %>%
  mutate(
...
#| label: data-visualisation - top 3 crimes by area map
#| message: false

# Plot the top 3 crime codes in each area map with jittered points
top_crime_codes_by_area_map <- top_crime_codes_by_area %>%
  ...
  labs(title = "Top 3 Crimes in Each Area", x = "Area", y = "Area Name") +
  theme_minimal() +
  theme(
...
```{r}
#| label: data-visualisation - abbreviations for area names
#| message: false
...
#| label: data-visualisation - number of top 3 crimes and total crimes by area using geom_sf with labels
#| message: false
...
# Plot the number of top 3 crimes and total crimes in each area map using geom_sf
top_crime_codes_by_area_map_sf <- ggplot() +
  ...
  theme(
    plot.title = element_text(size = 25, face = "bold", hjust = 0.5, margin = margin(b = -40)), # Adjust the size and position of the title
